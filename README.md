...
3. 在候选列表中用复选框勾选本次要处理的视频（默认全选），可右键排除 / 恢复、打开视频或目标 TXT 所在位置；列表会显示视频时长、文件大小与粗估处理时间。
4. 点击“开始补全并替换”。
...
- 候选列表支持复选框与全选；被排除候选显示“已排除”，本次不处理、不写回。
- 候选列表新增视频时长、文件大小与粗估处理时间列。
- 写回前备份，批量失败时自动回滚。
