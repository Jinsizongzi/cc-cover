...
1. 全量扫描
所有视频默认都是候选：无论同名 TXT 是否存在、是否为空或已有内容，都会重新生成并覆盖；
与视频不同名的 TXT 不会被触碰。
...
4. 自动替换字幕
点击“开始补全并替换”后，软件完成扫描、识别、校验并原子替换目标 TXT，无需额外写回参数。
...
• 运行设备：自动检测 GPU（有则默认 NVIDIA，否则 CPU），可在“运行环境”区域切换。
...
• 模型缓存：“运行环境”可“打开缓存位置”或“清理模型缓存”（清理前确认，清理后需重新下载）。
