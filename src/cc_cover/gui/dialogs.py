...
from cc_cover.core.pipeline import CompletionStats, SUMMARY_FILENAME, run_status_label
...
class DialogHost:
    ...
    def show_cleanup_dialog(self) -> None:
        """运行目录清理：勾选要删的运行目录，弹窗内容脱离 CCCoverApp 单独构造。"""
        runs = list_runs(self.runs_root)
        dialog = self.result_dialog("运行目录清理")
